...
<title>MedicineSearch - 서비스 해지 리스트</title>
...
<script type="text/javascript">
   		...
   		function selectChange() {
   			if(keyword != null){
				location = 'cancelList?page=' + page + '&numsPerPage=' + $('#numsPerPage').val() + '&keyword=' + keyword + '&category=' + category;
   			} else {
   				location = 'cancelList?page=' + page + '&numsPerPage=' + $('#numsPerPage').val();
   			}
		}
   		
   		function cancelDetail(seq){
   			document.cancelDetailForm.seq.value = seq
   			document.cancelDetailForm.action = '/cancelDetail'
...
<body>
<form id="cancelDetailForm" name="cancelDetailForm" method="post">
	 ...
	 <section  id="get-a-quote" class="get-a-quote">
		<div class="container" data-aos="fade-up">
			<div class="row g-0" id="dd">
				<div class="col-lg-5 quote-bg" style="background-image: url(resources/img/cancelList.jpg);">
				</div>
				<div class="col-lg-7" >
				<!-- cancelListView 시작 -->
				<div  id ="enquireListView" class="get-a-quote">
					<div class="col-lg-12">
						<p class="askTop">서비스 해지 문의 내역</p>
					</div>
							...
							<tbody>
								<c:forEach items="${List}" var="ask" varStatus="status">
									<tr style="padding:5px; text-align: center;" >
											...
											<td style="padding:5px;">
											<a onclick="cancelDetail('${ask.seq}')" style="cursor:pointer;"> ${ask.title}</a></td>
						   	...
						   	<c:when test="${pageMaker.criteria.keyword eq null}">
							   	<ul class="pagination justify-content-center">
									<c:if test="${pageMaker.hasPrev }">
										<li class="page-item"><a class="page-link" href="cancelList?page=${pageMaker.startPageNo - 1 }&numsPerPage=${pageMaker.criteria.numsPerPage}">&lt;</a></li>
									</c:if>
									<c:forEach begin="${pageMaker.startPageNo }"
										end="${pageMaker.endPageNo }" var="num">
										<li id="page${num}" class="page-item"><a class="page-link" href="cancelList?page=${num }&numsPerPage=${pageMaker.criteria.numsPerPage}">${num }</a></li>
									</c:forEach>
									<c:if test="${pageMaker.hasNext }">
										<li class="page-item"><a class="page-link" href="cancelList?page=${pageMaker.endPageNo + 1 }&numsPerPage=${pageMaker.criteria.numsPerPage}">&gt;</a></li>
									</c:if>
								</ul>
							</c:when>
							<c:otherwise>
								<ul class="pagination justify-content-center">
									<c:if test="${pageMaker.hasPrev }">
										<li class="page-item">
											<a class="page-link" href="cancelList?page=${pageMaker.startPageNo - 1 }&numsPerPage=${pageMaker.criteria.numsPerPage}&keyword=${pageMaker.criteria.keyword}&category=${pageMaker.criteria.category}">&lt;</a>
										</li>
									</c:if>
									<c:forEach begin="${pageMaker.startPageNo }"
										end="${pageMaker.endPageNo }" var="num">
										<li id="page${num}" class="page-item">
											<a class="page-link" id="page2${num}" href="cancelList?page=${num }&numsPerPage=${pageMaker.criteria.numsPerPage}&keyword=${pageMaker.criteria.keyword}&category=${pageMaker.criteria.category}">${num }</a>
										</li>
									</c:forEach>
									<c:if test="${pageMaker.hasNext }">
										<li class="page-item">
											<a class="page-link" href="cancelList?page=${pageMaker.endPageNo + 1 }&numsPerPage=${pageMaker.criteria.numsPerPage}&keyword=${pageMaker.criteria.keyword}&category=${pageMaker.criteria.category}}">&gt;</a>
										</li>
									</c:if>
								</ul>
							</c:otherwise>
